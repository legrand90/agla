...
import 'package:lavage/authentification/Screen/Listes/listcouleurs.dart';
...
class EditCouleur extends StatefulWidget {
  var idcouleur ;
  EditCouleur({Key key, @required this.idcouleur}) : super(key: key);

  @override
  _EditCouleurState createState() => new _EditCouleurState(this.idcouleur);
}
class _EditCouleurState extends State<EditCouleur> {
  var idcouleur ;
  _EditCouleurState(this.idcouleur);
  ...
  void UpdateCouleur() async{
    ...
    var id_user = localStorage.getInt('ID');
      ...
      //try {
      var data = {
        'libelle_couleur': _nomCouleur.text,
       // 'id_lavage': id
      };

    var dataLog = {
      'fenetre': '$fenetre',
      'tache': "Modification d'une Couleur",
      'execution': "Update",
      'id_user': id_user,
      'dateEnreg': date,
      'id_lavage': id,
  ...
  void getCouleur() async {
    SharedPreferences localStorage = await SharedPreferences.getInstance();
    var idlavage = localStorage.getString('id_lavage');
    // this.param = _mySelection3;

    //final String url = "http://192.168.43.217:8000/api/getCouleur/$idcouleur/$idlavage"  ;
...
//    });
    ...
    setState(() {
      _nomCouleur.text = resBody['couleur'];
