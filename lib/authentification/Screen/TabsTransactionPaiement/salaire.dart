...
import 'package:lavage/authentification/Models/listSalaire.dart';
...
class SalaireAgent extends StatefulWidget {

  final Widget child ;

  SalaireAgent({Key key, @required this.child}) : super(key: key);

  @override

  _SalaireAgentState createState() => _SalaireAgentState();
}

class _SalaireAgentState extends State<SalaireAgent> {
...
//    });
  ...
  var solde, reste, totalAPayer;

  Future<String> getSalaireAgent() async {
    SharedPreferences localStorage = await SharedPreferences.getInstance();
    var id = localStorage.getString('id_lavage');
    var res = await CallApi().getData('getAgentSalire/$_mySelection/$id');
    //final String urlAgent = "http://192.168.43.217:8000/api/agent/$id";
    ...
    setState(() {
      solde = resBody['salaire'];
      reste = resBody['reste'];
      totalAPayer = resBody['TotalAPayer'];
      //_mySelection = resBody[0]['id'];
      visible = true;
    });
          ...
          child: ListView(
            children: <Widget>[
              ...
              Container(
                ...
                Row(
                  children: <Widget>[
                    Text('Reste : '),
                    //SizedBox(width: 20.0,),
                    Expanded(child: Text('  $reste FCFA'),)
                  ],
                ),
              ...
              Container(
                ...
                Row(
                  children: <Widget>[
                    Text('Total à Payer : '),
                    //SizedBox(width: 20.0,),
                    Expanded(child: Text('  $totalAPayer FCFA'),)
                  ],
                ),
              ...
              visibleListPaiement? Container(child: Center(
                child: Text("5 dernières paiements ", style: TextStyle(fontSize: 20.0),),
              ),) : Text(''),
              SizedBox(height: 10.0,),
              visibleListPaiement? Container(
                  ...
                  ListView.builder(
                    shrinkWrap: true,
                    itemCount: (listsalaire == null || listsalaire.data == null || listsalaire.data.length == 0 )? 0 : listsalaire.data.length,
                              ...
                              children: <Widget>[
                                Row(
                                  children: <Widget>[
                                    Text('DATE : '),
                                    SizedBox(width: 20.0,),
                                    Text('${listsalaire.data [index] .dateEnreg}'),
                                  ],
                                ),
                                SizedBox(height: 10.0,),
                                Row(
                                ...
                                Row(
                                  children: <Widget>[
                                    Text('MONTANT PAYE'),
                                    SizedBox(width: 20.0,),
                                    Text('${listsalaire.data [index] .montant} Fcfa'),
                                  ],
                                ),
  ...
  Listsalaire listsalaire = Listsalaire();
  ...
  void _sendDataPaiement() async{
    ...
    if(validateAndSave()) {
      ...
      var data = {
        ...
        'reste': ((totalAPayer) - int.parse(montant.text)),
      };
      ...
      if((totalAPayer) >= int.parse(montant.text)) {
        var res = await CallApi().postData(data, 'create_salaire');
        ...
        if (res.statusCode == 200) {
          ...
          var resTrans = await CallApi().getData(
              'getLast5TansactionPaiementAgent/$id/$_mySelection');

          await getSalaireAgent();

          setState(() {
            //solde = int.parse(solde) - int.parse(montant.text);
            listsalaire = listsalaireFromJson(resTrans.body);
            montant.text = '';
        ...
        _showMsg("Désolé...Le montant entré ne peut être supérieur au montant total à payer !");
      }
