...
class SmsConfig extends StatefulWidget {
  @override
  _SmsConfigState createState() => new _SmsConfigState();
}
...
class _SmsConfigState extends State<SmsConfig> {
  ...
  final TextEditingController _alerte = TextEditingController();
  ...
  //DateTime selectedDate = DateHeure.now ();
  ...
  Future<Null> _selectTime(BuildContext context) async {
    ...
    print('heure : ${_timeController.text}');
  }
  ...
  void DesactivateUser() async{
    ...
    SharedPreferences localStorage = await SharedPreferences.getInstance();
    var id = localStorage.getString('id_lavage');
    var res = await CallApi().postDataEdit(data, 'desactiverUser');
  ...
  void smsConfig() async{
    ...
    if(resBody['on_off'] == 'on'){
      setState(() {
        _switchVal = true;
        success = resBody['success'];
        on_off = resBody['on_off'];
        _alerte.text = resBody['alerte'];
        heure = resBody['heure'];
      });
    }else if(resBody['on_off'] == 'off'){
      setState(() {
        _switchVal = false;
        success = resBody['success'];
        on_off = resBody['on_off'];
    ...
    _timeController.text = _switchVal ? heure : formatDate(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Text("ACTIVEZ OU DESACTIVEZ LE SERVICE SMS",
                      textAlign: TextAlign.center,
                      style: TextStyle(
                          fontSize: 20.0,
  ...
  void _sendDataSmsConfig() async {
    ...
    var idUser = localStorage.getInt('ID');
      ...
      var res = await CallApi().postAppData(data, 'updateSmsconfig/$id');
      var body = json.decode(res.body);
      //print('$body');
