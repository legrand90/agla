...
import 'package:lavage/authentification/Models/smsopera.dart';
...
class RechercheSms extends StatefulWidget {

  final Widget child ;

  RechercheSms({Key key, @required this.child}) : super(key: key);

  @override

  _RechercheSmsState createState() => _RechercheSmsState();
}

class _RechercheSmsState extends State<RechercheSms> {
  ...
  //Smsopera serchValue = Smsopera();
  Smsopera serchValue2 = Smsopera();
  ListagentNumTrans serchValue3 = ListagentNumTrans();
  var data ;
  List data2 = List() ;

  void getSmsSendFromSearchTransa() async {
    var resBody;
    SharedPreferences localStorage = await SharedPreferences.getInstance();
    var id = localStorage.getString('id_lavage');
    //var resBody = listclientsearchFromJson(res.body);
    var res2 = await CallApi().getData('getSmsSend/$id/$datEnreg1/$datEnreg2');
...
//    });
    ...
    setState(() {
      serchValue2 = smsoperaFromJson(res2.body);
    });
  ...
  void getLast10Sms() async {
    var resBody;
    SharedPreferences localStorage = await SharedPreferences.getInstance();
    var id = localStorage.getString('id_lavage');
    //var resBody = listclientsearchFromJson(res.body);
    var res2 = await CallApi().getData('getLast10Sms/$id');
    // final String urlTrans = "http://192.168.43.217:8000/api/getAgentFromSearchTrans/$id/${serchValue.data.id}/$datEnreg1/$datEnreg2";
//    final res2 = await http.get(Uri.encodeFull(urlTrans), headers: {
//      "Accept": "application/json",
//      "Content-type": "application/json",
                              ...
                              children: <Widget>[
                                Row(
                                  children: <Widget>[
                                    Text('SMS : '),
                                    SizedBox(width: 20.0,),
                                    Expanded(child:Text('${serchValue2.data [index] .contenu}')),
